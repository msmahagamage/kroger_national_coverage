...
nb3 = notebook(
    ...
    [
        nbf.v4.new_code_cell("import pandas as pd\nfrom build_all import build_map\n\nprint('[1/4] Loading official location and state-summary files...')\nlocations = pd.read_csv(ROOT / 'data' / 'processed' / 'kroger_official_locations.csv', dtype={'zip_code': 'string'}).fillna('')\nstate_summary = pd.read_csv(ROOT / 'data' / 'processed' / 'state_summary.csv')\nprint(f'[2/4] Loaded {len(locations):,} locations across {len(state_summary)} states.')\nprint('[3/4] Building the interactive web map; this can take a moment...')\nbuild_map(locations, state_summary)\nprint('[4/4] Complete: created docs/index.html')"),
